...
pub trait AddData {
    ...
    async fn update_metrics(&self, packet: Pkt) -> Result<u64, Error>;
    ...
    async fn update_device_met(&self, data: DeviceMetrics) -> Result<u64, Error>;
    async fn update_user(&self, data: User) -> Result<u64, Error>;
    async fn update_node_pos(&self, data: Position) -> Result<u64, Error>;
}
...
impl AddData for tokio_postgres::Client {
    async fn update_metrics(&self, packet: Pkt) -> Result<u64, Error> {
        match packet {
            ...
            Pkt::NInfo(ni) => {
                ...
                let mut rv = 0;
                if let Some(data) = ni.device_metrics {
                    // need to change to handle errors
                    rv += self.update_device_met(data).await.unwrap();
                }
                if let Some(data) = ni.user {
                    // need to change to handle errors
                    rv += self.update_user(data).await.unwrap();
                }
                if let Some(data) = ni.position {
                    // need to change to handle errors
                    rv += self.update_node_pos(data).await.unwrap();
                }
                Ok(rv)
            }
            _ => {
    ...
    async fn add_environmental_metrics(
        ...
        INSERT INTO environmentmetrics (
            msg_id, node_id, date, time, tempurature, relative_humidity, barometric_pressure, 
            gas_resistance, iaq
        ) 
